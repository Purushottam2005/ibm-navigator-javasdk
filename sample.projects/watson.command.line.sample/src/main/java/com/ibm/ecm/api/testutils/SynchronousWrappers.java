...
import com.ibm.ecm.api.coresdk.model.IBMECMComment;
...
public class SynchronousWrappers {
    ...
    public static IBMECMContentItem retrieveItem(IBMECMRepository repository, String itemId) {
        Holder<IBMECMContentItem> returnVal = new Holder<IBMECMContentItem>(null);
        final CountDownLatch signal = new CountDownLatch(1);
        repository.retrieveItem(itemId, new IBMECMCompletionHandler<IBMECMContentItem>() {
            private Holder<IBMECMContentItem> anonHolder;
        ...
        }.init(returnVal));  //  initialize the inner class with the return variable
    ...
    public static IBMECMComment addComment(String commentText, IBMECMContentItem contentItem) {
        final Holder<IBMECMComment> returnVal = new Holder<IBMECMComment>(null);
        final CountDownLatch signal = new CountDownLatch(1);
        contentItem.addComment(commentText, new IBMECMCompletionHandler<IBMECMComment>() {
            @Override
            public void onCompleted(IBMECMComment commentItem) {
                returnVal.setValue(commentItem);
                signal.countDown();
            }
            ...
            public void onError(IBMECMRuntimeException exp) {
                signal.countDown();
                System.out.format("IBMECMRuntimeException | .code = %s,.message = %s\n", exp.getErrorCode() != null? exp.getErrorCode().getCode() : null,
                        exp.getMessage());
            }
        });
